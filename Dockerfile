...
# The purpose of this image is to be able to host ElectrumX for Lambdad (RXD)
# Build with: `docker build -t electrumx .`
# Public images at: https://hub.docker.com/repository/docker/Lambdablockchain

FROM ubuntu:22.04

LABEL maintainer="Lambdablockchain@protonmail.com"
LABEL version="1.2.0"
LABEL description="Docker image for electrumx Lambdad node"
...
# ORIGINAL SOURCE
RUN git clone --depth 1 --branch master https://github.com/Lambdablockchain/electrumx.git

WORKDIR /root/electrumx

RUN python3 -m pip install -r requirements.txt

ENV DAEMON_URL=http://dockeruser:dockerpass@localhost:9332/
ENV COIN=Lambda
...
RUN openssl req -new -key server.key -out server.csr -subj "/C=US/ST=Denial/L=Springfield/O=Dis/CN=Lambdablockchain.org"
